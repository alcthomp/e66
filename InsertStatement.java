        ...
        try {
            ...
            try {
            	if(row.getKey() == null ){
	                ...
	                ret = table.getDB().putNoOverwrite(null, null , row.getData()); 
            	}else {
	                ret = table.getDB().putNoOverwrite(null, row.getKey(), row.getData()); 
            	}
            } catch (DatabaseException de) {
                System.err.println("error occurred while storing record");
                return;
            } 
            if (ret == OperationStatus.KEYEXIST){
            	System.err.println("DBMS already has this primary key already.");
            }
            ...
            System.err.println("Could not insert row." + e);
        }
    }
}
